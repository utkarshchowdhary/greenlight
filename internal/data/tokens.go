...
// plaintext and hashed versions of the token, associated userID, expiry time and
// scope.
type Token struct {
	Plaintext string
	Hash      []byte
...
func generateToken(userID int64, ttl time.Duration, scope string) (*Token, error) {
	...
	// The sha256.Sum256() function returns an *array* of length 32
	hash := sha256.Sum256([]byte(token.Plaintext))

	// To make it easier to work with we convert it to a slice using the [:] operator
	// before storing it.
	token.Hash = hash[:]
