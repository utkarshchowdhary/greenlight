...
// Define a User struct to represent an individual user. Use the json:"-" struct tag
// to prevent the Password and Version fields appearing in any output when we encode
// it to JSON. Also the Password field uses the custom password type defined below.
type User struct {
	ID        int64     `json:"id"`
...
// Declare a AnonymousUser variable.
...
// Insert a new record in the database for the user. The id, created_at and
...
func (m UserModel) Insert(user *User) error {
	...
	// constraint. We check for this error specifically, and return custom ErrDuplicateEmail
	// error instead.
...
// And we also check for a violation of the "users_email_key" constraint
...
func (m UserModel) GetForToken(tokenScope, tokenPlaintext string) (*User, error) {
	...
	// is not supported by the pq driver), and we pass the current time as the
